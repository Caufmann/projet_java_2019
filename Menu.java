...
public class Menu extends JFrame implements ActionListener, ItemListener {
    ...
    public Menu() {
        super (" Projet : Gestion d'une Ecole");
        ...
        /**
         * CrÃ©ation des Listes pour les tables
         */
    ...
    public void actionPerformed (ActionEvent evt){
        ...
        if (source == connect){
            ...
            String nameBdd = "gestion_ecole";//new String (nameBDDTexte.getText());
            try {
                try {
                    /**
                   ...
                   System.out.println( maconnexion.remplirChampsRequete("SELECT nom_ecole FROM Ecole WHERE id_ecole = 1"));
                    ...
                    Controleur<Eleve> eleveDao = new EleveDAO(maconnexion);
                    for(int i = 1; i < 6; i++){
                        Eleve eleve = eleveDao.find(i);
                        JLabel Eleve = new JLabel("ElÃ¨ve NÂ°" + eleve.getId() + "  - " + eleve.getNom() + " " + eleve.getPrenom());
                        System.out.println("ElÃ¨ve NÂ°" + eleve.getId() + "  - " + eleve.getNom() + " " + eleve.getPrenom());
                        p2.add(Eleve);
                        add(p2);
                    }

                    /**
                     * Si la connexxion est un succÃ¨s, alors on appelle la classe Controleur<AnnÃ©eScolaire>
                     *     qui hÃ©rite de la classe Controleur
                     *     On affiche ainsi toutes les annÃ©es de l'Ã©cole prÃ©sentes dans la base de donnÃ©e
                     */
                    Controleur<AnneeScolaire> yearDao = new AnneeScolaireDAO(maconnexion);
                ...
                Update upd = new Update("personne");
        } else if (source == JBclasse) {
            ClassView classe = new ClassView("test");
        }
    }
    ...
    public void itemStateChanged (ItemEvent e){
        ...
        if (e.getSource() == ListYear){
            ...
            Controleur<AnneeScolaire> AnneeSelect = new AnneeScolaireDAO(maconnexion);
            for (int z = 0; z< 4; z++) {
            ...
            /**
             * Affiche tous les niveaux disponible de la base de donnÃ©es
             */
       ...
       if (e.getSource() == ListNiveau){
            ...
            Controleur<Classe> classeDAO = new ClasseDAO(maconnexion);
            for (int i=1; i< 6; i++) {
               System.out.println("Id de l'annÃ©e :" + anneeSelelect.get(0).getId() + " Id du niveau " + niveauSelelect.get(0).getId());
       ...
       if (e.getSource() == ListClasse) {
           ...
           /**
            * On appelle la classe EleveDAO
            * On utlise la methode FIND
            * requete en fonction de l'ID de la classe de l'eleve
            */
